...
from data_processor import (
    ...
    get_growth,
    calculate_market_share_over_time # Import the new function
)
...
with st.sidebar:
    ...
    # Limit default selection for a cleaner initial view
...
if filtered_df.empty:
    ...
    # --- NEW: High-Level Market Trends ---
    st.subheader("High-Level Market Trends")
    # Using tabs to organize the new charts cleanly
    tab1, tab2 = st.tabs(["Trends by Vehicle Type", "Market Share Evolution"])

    with tab1:
        # This chart shows which vehicle segment (2W, 4W) is growing or shrinking
    ...
    with tab2:
        # This chart shows if manufacturers are gaining or losing market share over time
    ...
    # --- Existing Charts for Manufacturer-Specific Analysis ---
    st.subheader("Manufacturer Performance Analysis")
    
    col_pie, col_placeholder = st.columns([2, 1]) # Give pie chart more space
    with col_pie:
